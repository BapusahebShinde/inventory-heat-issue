/**
 * i-Tek RFID - SGTIN96 Encoder/Decoder Library
 *
 * Author: Saurav Jain
 */
...
bool sgtin96_validate_epc_hex(const char *epc) {
...
PartitionInfo sgtin96_get_partition_info(int p) {
...
EncodeResult sgtin96_encode(const char *gtin14, const char *serial) {
    ...
    char norm_gtin14[15];
    if (!sgtin96_validate_gtin14(gtin14, norm_gtin14)) {
    ...
    int partition = sgtin96_determine_partition(gcp);
    if (partition < 0) {
        snprintf(r.message, sizeof(r.message), "Invalid GCP length");
        return r;
    }

    PartitionInfo p = sgtin96_get_partition_info(partition);
    ...
    int pos = 0;
    insert_bits(bin, pos, SGTIN96_HEADER, 8); pos += 8;
...
DecodeResult sgtin96_decode(const char *epc_hex) {
    ...
    if (!sgtin96_validate_epc_hex(epc_hex)) {
        snprintf(r.message, sizeof(r.message), "Invalid EPC hex");
        return r;
    }

    char bin[97];
    hex_to_binary(epc_hex, SGTIN96_EPC_CHARS, bin);
    ...
    int cd = sgtin96_calculate_check_digit(gtin13);
...
bool sgtin96_gtin14_to_gtin12(const char *gtin14, char *out) {
    char norm[15];
    if (!sgtin96_validate_gtin14(gtin14, norm))
        return false;
...
bool sgtin96_gtin12_to_gtin14(const char *gtin12, char *out) {
