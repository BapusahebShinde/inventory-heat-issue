...
import com.itek.retail.databinding.FragmentEncodingStartBinding;
 ...
 */
public class EncodingStartFragment extends RFIDSessionFragment {
    ...
    private FragmentEncodingStartBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        binding = FragmentEncodingStartBinding.inflate(inflater, container, false);
        ...
        /*final ConstraintLayout root = (ConstraintLayout) binding.clStartEncodingScore;
        ...
        if(total <= 0)
          constraintSet.connect(binding.txtEncodingStartScore.getId(), ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP);
        constraintSet.connect(binding.txtEncodingStartScore.getId(), ConstraintSet.BOTTOM, total > 0 ? binding.divEncodingStartScore.getId() : ConstraintSet.PARENT_ID, total > 0 ? ConstraintSet.TOP : ConstraintSet.BOTTOM);
        ...
        binding.llBtnStart.setOnClickListener(v -> {
            ...
            if (!chkNotNullTrue(isSessionOn) && sessionObject == null)
                ...
                if (chkNotNullTrue(mainViewModel.getIsInventoryOn().getValue()))
                    ...
                    AppCommonMethods.showLog(EncodingStartFragment.this.getClass().getSimpleName(), "llBtnStart->Clicked");
                    pickedTag = null;
        ...
        binding.listEncodingStartHistory.setAdapter(new EncodingHistoryAdapter((MainActivity) context, EncodingStartFragment.this, listEncodedEans));
            ...
            public void onClick(View v) {
                if (v != null && v.getVisibility() == View.VISIBLE && SharedPrefManager.getBoolean(ParamConstants.IS_ENC_VERIFY) && chkNotNullTrue(mainViewModel.getIsSessionOn().getValue()) && sessionObject != null && !isProcessOn() && checkReaderConnected() && getSize() > 0) {
                    ...
                    if (!isVerifyEncodeRedirection) mainViewModel.startInventory();
                        ...
                        Bundle args = EncodingStartFragment.this.getArguments();
        ...
        binding.listVerifyEncoded.setAdapter(new OmniPickedListAdapter(context, EncodingStartFragment.this, listEncodedTags));
    ...
    public void setTarget(int target) {
        ...
        try {
            target = sessionObject != null ? AppCommonMethods.parseInt(sessionObject.total) : chkNull(EncodingStartFragment.this.target, -1) >= 0 ? EncodingStartFragment.this.target : target;
    ...
    void setBarcodeObservers() {
      ...
      binding.divFifo.setEnabled(!isProcessOn);*/
        ...
        mainViewModel.getBarcodeData().observe(getViewLifecycleOwner(), barcode -> {
            showLog(EncodingStartFragment.this.getClass().getSimpleName() + "_barcodeData", chkNull(barcode, ""));
            if (!isTopInStack()) return;
        ...
        if (inventoryDao != null && sessionObject != null && SharedPrefManager.getBoolean(ParamConstants.IS_ENC_VERIFY)) {
            ...
            if (!isRemove)
                inventoryDao.getEncVerifiedCount(sessionObject.sessionId).observe(getViewLifecycleOwner(), new Observer<MultiQtyModel>() {
                    ...
                    public void onChanged(MultiQtyModel multiQtyModel) {
                        if (multiQtyModel != null && multiQtyModel.found != null && multiQtyModel.total != null) {
              /*if(binding != null && binding.txtEncodingStartScore != null)
                binding.txtEncodingStartScore.setText(multiQtyModel.found + " / " + multiQtyModel.total);*/
                            if (binding != null && binding.ctwInventoryStart != null)
                                binding.ctwInventoryStart.setScore(multiQtyModel.found, multiQtyModel.found + " / " + multiQtyModel.total);
    ...
    public void apiCall(String action, Bundle args) {
        ...
        if (!isUseAPICallForSessionEncode && !SharedPrefManager.getBoolean(ParamConstants.IS_OFFLINE_ENCODE) && isInternetConnected(context, false, isUpload)) {
            try {
                ...
                callWebService(isUpload ? isDCApp ? URLConstants.UPLOAD_ENCODE : URLConstants.UPLOAD_ENCODING : URLConstants.SET_SESSION, requestParams, args, isUpload, isUpload ? getString(R.string.progress_msg_uploading_data) : "", !isUpload, false);
            } catch (JSONException e) {
                e.printStackTrace();
                hideProgressDialog();
            }
    ...
    public void toggleVerifyMode(final boolean isVerifyMode) {
        EncodingStartFragment.this.isVerifyMode = isVerifyMode;
        ...
        try {
            ...
            if (isCheckPasswordBeforeAPI && (!SharedPrefManager.getBoolean(ParamConstants.IS_CHECK_PASSWORD_BEFORE_BASED_ON_EPC, AppCommonMethods.isCheckEncPasswordBasedOnEPC) || isCheckPasswordBasedOnEPC) && SharedPrefManager.getDeviceTypeValue() == AppCommonMethods.DeviceType.ZEBRA.getValue()) {
                ...
                if (AppCommonMethods.isStaticDebug()) {
                    ...
                    handleResponse(URLConstants.GET_EPC_FOR_ENCODING, requestParams, jsonResponse, 200, true, getArguments());
                } else {
                    final String url = getEPCForEncodeUrl();
    ...
    protected void onDataSizeChanged(Integer size) {
        ...
        if (binding != null) {
            ...
            if (size > 0 && SharedPrefManager.getBoolean(ParamConstants.IS_ENC_VERIFY)) { // if isShow
                ...
                if (binding.ctwInventoryStart != null)
                    binding.ctwInventoryStart.setScore(chkNull(size, 0), verifiedTags + " / " + chkNull(size, 0));
      ...
      binding.progressEncodingStart.setProgress(percentage);*/
            ...
            //if (binding.ctwInventoryStart != null) binding.ctwInventoryStart.setScoreColor(size,binding.ctwInventoryStart.getScore());
        }
    }
            ...
            switch (url) {
                ...
                case URLConstants.GET_EPC_FOR_ENCODE:
                    ...
                    if (isSuccess) {
                        ...
                        final String referenceId = extractString(jsonResponse, ParamConstants.REFERENCE_ID, extractString(jsonResponse, ParamConstants.REFERENCE_EAN, extractString(jsonResponse, ParamConstants.REFERENCE_BARCODE, ""))).trim();
                        if (pickedTag != null && isNonEmpty(newEpc)) {
                            ...
                            if (isNonEmpty(referenceId) || isNonEmpty(articleNo)) {
                                args = chkNull(args, new Bundle());
                                args.putString(ParamConstants.ARTICLE_NO, articleNo);
                                args.putString(ParamConstants.REFERENCE_ID, referenceId);
                ...
                case URLConstants.UPLOAD_ENCODE:
                    ...
                    if (isSuccess) {
                        final long total = Long.parseLong(chkNull(binding.ctwInventoryStart.getTotal(), "0"));
