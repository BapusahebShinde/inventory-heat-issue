...
import com.itek.retail.databinding.FragmentOutwardMainBinding;
 ...
 */
public class OutwardMainFragment extends CommonFragment{
  
  private OutwardMainViewModel mViewModel;
  private FragmentOutwardMainBinding binding;
   ...
   */
  public OutwardMainFragment(){/*Default/Empty Constructor*/}
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState){
    mViewModel = new ViewModelProvider(this, (ViewModelProvider.Factory) new ViewModelProvider.NewInstanceFactory()).get(OutwardMainViewModel.class);
    binding = FragmentOutwardMainBinding.inflate(inflater, container, false);
    ...
    binding.header.imgConfigSync.setVisibility(true ? View.VISIBLE : View.GONE);
  ...
  private void callAPI(){
    if(true){
      callWebService(URLConstants.GET_OUTWARD_DASHBOARD, new JSONObject(), getString(R.string.progress_msg_getting_data));
    }
    else{
      final int totalTrips = chkNull(tripStatusDao.getTripsAllCount(AppConstants.OUTWARD), 0);
      final int completeTrips = chkNull(tripStatusDao.getTripsCompletedCount(AppConstants.OUTWARD), 0);
      final int pendingTrips = chkNull(totalTrips - completeTrips, 0);
      
      if(binding != null && binding.dvOtwMainListTotal != null)
        binding.dvOtwMainListTotal.setText("" + totalTrips);
      if(binding != null && binding.dvOtwMainListCompleted != null)
        binding.dvOtwMainListCompleted.setText("" + completeTrips);
      if(binding != null && binding.dvOtwMainListPending != null)
        binding.dvOtwMainListPending.setText("" + pendingTrips);
      ...
      if(binding != null && binding.dvOtwMainHusTotal != null)
        binding.dvOtwMainHusTotal.setText("" + totalHus);
      if(binding != null && binding.dvOtwMainHusCompleted != null)
        binding.dvOtwMainHusCompleted.setText("" + completeHUs);
      if(binding != null && binding.dvOtwMainHusPending != null)
        binding.dvOtwMainHusPending.setText("" + pendingHus);
    }
  }
   ...
   */
  private void clearDashboardVals(){ clearDashboardVals(binding.llDashboardOtw); }
  
  private void clearDashboardVals(final LinearLayout ll){
    ...
    else if(ll.getId() == binding.llDashboardOtw.getId()){
      if(binding.dvOtwMainListTotal != null) binding.dvOtwMainListTotal.setText(defNoVal);
      if(binding.dvOtwMainListPending != null) binding.dvOtwMainListPending.setText(defNoVal);
      if(binding.dvOtwMainListCompleted != null) binding.dvOtwMainListCompleted.setText(defNoVal);
      if(binding.dvOtwMainHusTotal != null) binding.dvOtwMainHusTotal.setText(defNoVal);
      if(binding.dvOtwMainHusPending != null) binding.dvOtwMainHusPending.setText(defNoVal);
      if(binding.dvOtwMainHusCompleted != null) binding.dvOtwMainHusCompleted.setText(defNoVal);
    }
  }
  ...
  public void handleResponse(String url, JSONObject jsonRequest, JSONObject jsonResponse, Integer responseCode, boolean isSuccess, Bundle args){
    try{
      switch(url){
        case URLConstants.GET_OUTWARD_DASHBOARD:
          if(isSuccess){
            final String defNoVal = getString(R.string.default_no_value);
            ...
            if(isNonEmpty(jsonItems)){
              binding.llDashboardOtw.removeAllViews();
              for(int i = 0; i < jsonItems.length(); i++){
                try{
                  ...
                  binding.llDashboardOtw.addView(llDashboardColumn, llParams);
                }
                catch(Exception e){
                  e.printStackTrace();
                }
            ...
            //Old Code (Static Dashboard (only Label & Count changes))
          }
