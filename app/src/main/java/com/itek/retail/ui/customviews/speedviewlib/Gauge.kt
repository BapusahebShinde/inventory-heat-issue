...
import com.itek.retail.ui.customviews.speedviewlib.components.Section
import com.itek.retail.ui.customviews.speedviewlib.components.Style
import com.itek.retail.ui.customviews.speedviewlib.util.OnSectionChangeListener
import com.itek.retail.ui.customviews.speedviewlib.util.OnSpeedChangeListener
import com.itek.retail.ui.customviews.speedviewlib.util.doOnSections
import java.util.*
...
abstract class Gauge constructor(
  ...
  private var defMoveDuration = 500L
  ...
  private fun init() {
    textPaint.color = 0x00FFFFFF.toInt()
    textPaint.textSize = spToPx(0.0f);
    textPaint.textAlign = Paint.Align.CENTER
    speedTextPaint.color = 0xFF686868.toInt()
    speedTextPaint.textSize = spToPx(14.0f)
    speedTextPaint.typeface =
      Typeface.create(ResourcesCompat.getFont(context, R.font.roboto_regular), Typeface.BOLD)
    unitTextPaint.color = 0xFF686868.toInt()
    unitTextPaint.textSize = spToPx(14.0f)
    unitTextPaint.typeface =
      Typeface.create(ResourcesCompat.getFont(context, R.font.roboto_regular), Typeface.BOLD)
  ...
  private fun initAttributeSet(context: Context, attrs: AttributeSet?) {
    ...
    speedTextPaint.color = a.getColor(
      R.styleable.Gauge_sv_speedTextColor,
      ContextCompat.getColor(context, R.color.txt_regular)
    )
    speedTextPaint.textSize = a.getDimension(
      R.styleable.Gauge_sv_speedTextSize,
      context.resources.getDimension(R.dimen.sp_14)
    )
    speedTextPaint.typeface =
      Typeface.create(ResourcesCompat.getFont(context, R.font.roboto_regular), Typeface.BOLD)
    unitTextPaint.color = a.getColor(
      R.styleable.Gauge_sv_unitTextColor,
      ContextCompat.getColor(context, R.color.txt_regular)
    )
    unitTextPaint.textSize = a.getDimension(
      R.styleable.Gauge_sv_unitTextSize,
      context.resources.getDimension(R.dimen.sp_14)
    )
    unitTextPaint.typeface =
      Typeface.create(ResourcesCompat.getFont(context, R.font.roboto_regular), Typeface.BOLD)
    ...
    val speedFormat = a.getInt(R.styleable.Gauge_sv_speedTextFormat, 0)
    if (speedFormat == 0)
  ...
  private fun getSpeedUnitTextWidth(): Float =
    if (unitUnderSpeedText)
      max(speedTextPaint.measureText(getSpeedText().toString()), unitTextPaint.measureText(" %"))
    else
      speedTextPaint.measureText(getSpeedText().toString()) + unitTextPaint.measureText(" %") + unitSpeedInterval
  ...
  private fun updateSpeedUnitTextBitmap(speedText: String) {
    ...
    if (unitUnderSpeedText) {
      ...
      speedUnitTextCanvas?.drawText(speedText.split(".")[0], speedX, h, speedTextPaint)
   ...
   */
  @JvmOverloads
  fun speedPercentTo(percent: Int, moveDuration: Long = defMoveDuration) {
   ...
   * This value cannot be negative (defMoveDuration by default).
   *
   * @see speedTo
   * @see speedPercentTo
   * @see realSpeedTo
   */
  @JvmOverloads
  fun speedTo(speed: Float, moveDuration: Long = defMoveDuration) {
    var newSpeed = speed
    newSpeed =
      if (newSpeed > maxSpeed) maxSpeed else if (newSpeed < minSpeed) minSpeed else newSpeed
