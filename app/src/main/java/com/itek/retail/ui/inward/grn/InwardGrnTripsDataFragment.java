...
import com.itek.retail.databinding.FragmentInwardGrnTripsDataBinding;
...
/**
 * The Inward grn trips data fragment.
 */
public class InwardGrnTripsDataFragment extends CommonFragment{
  
  private TripStatusDao tripStatusDao;
  private TripInventoryDao tripInventoryDao;
  private FragmentInwardGrnTripsDataBinding binding;
  private InwardGrnTripsDataViewModel mViewModel;
  
  /**
   * Instantiates a new Inward grn trips data fragment.
   */
  public InwardGrnTripsDataFragment(){/*Default/Empty Constructor*/}
  ...
  public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
    binding = FragmentInwardGrnTripsDataBinding.inflate(inflater, container, false);
    mViewModel = new ViewModelProvider(this, (ViewModelProvider.Factory) new ViewModelProvider.NewInstanceFactory()).get(InwardGrnTripsDataViewModel.class);
    
    final List<String> listTripTypeTabs = tripStatusDao != null ? tripStatusDao.getTripTypes(AppConstants.INWARD) : new ArrayList<>(0);
    
    binding.viewPager.setAdapter(new TripTypesPagerAdapter(this, listTripTypeTabs));
    new TabLayoutMediator(binding.tabLayout, binding.viewPager, (tab, position) -> tab.setText((listTripTypeTabs.get(position).contains("_")?AppCommonMethods.toTitleCase(listTripTypeTabs.get(position)):listTripTypeTabs.get(position)).replaceAll(" To ", " to "))).attach();
    ...
    binding.btnUpload.setOnClickListener(view -> {
      ...
      if(binding.btnSwipeUpload != null && binding.btnSwipeUpload.getVisibility() == View.VISIBLE)
        return;
      showTripCompleteConfirmDialog(tripStatusDao.getCompletedTrips(AppConstants.INWARD));
    });
        ...
        try{
          ...
          if(isNonEmpty(listTripNos)){
            ...
            for(String tripNo : listTripNos){
              ...
              jobj.put(ParamConstants.K_TRIP_TYPE, tripStatusDao.getTripType(tripNo, AppConstants.INWARD));
            ...
            callWebService(URLConstants.UPLOAD_INWARD, requestParams, args, getString(R.string.progress_msg_uploading_data));
          }
        }
  ...
  public void updateLists(){
    ...
    if(binding != null && binding.btnUpload != null && binding.btnSwipeUpload != null){
      binding.btnUpload.setVisibility(tripStatusDao.getCompletedTripsCount(AppConstants.INWARD) > 0 ? View.VISIBLE : View.GONE);
      binding.btnSwipeUpload.setVisibility(AppCommonMethods.isUploadSlider && tripStatusDao.getCompletedTripsCount(AppConstants.INWARD) > 0 ? View.VISIBLE : View.GONE);
    }
  }
  
  @Override
  public void onBackPressed(){
    if(!tripStatusDao.isAllTripsPending(AppConstants.INWARD)){
      String counts = tripStatusDao.getTripsStatusCounts(AppConstants.INWARD);
          ...
          if(isSuccess){
            ...
            if(completedTrips != null && completedTrips.length() > 0){
              ...
              if(isNonEmpty(completedTripNos)){
                tripStatusDao.deleteUploadAllTripStatus(completedTripNos, AppConstants.INWARD);
            ...
            if(isNonEmpty(completedTripNos))
              context.showCustomSuccessDialog(msg.trim(), tripStatusDao.getTripsCount(AppConstants.INWARD) == 0);
