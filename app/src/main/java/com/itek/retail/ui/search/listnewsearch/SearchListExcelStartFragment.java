...
import static com.itek.retail.common.AppCommonMethods.isCallAPIBasedMarkFound;
...
import com.itek.retail.databinding.FragmentSearchListExcelStartBinding;
 ...
 */
public class SearchListExcelStartFragment extends RFIDSessionFragment{
  
  private SearchListExcelStartViewModel mViewModel;
  private FragmentSearchListExcelStartBinding binding;
   ...
   */
  public SearchListExcelStartFragment(){
  ...
  public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
    mViewModel = new ViewModelProvider(this, (ViewModelProvider.Factory) new ViewModelProvider.NewInstanceFactory()).get(SearchListExcelStartViewModel.class);
    binding = FragmentSearchListExcelStartBinding.inflate(inflater, container, false);
    ...
    searchListId = extractString(getArguments(), ParamConstants.SEARCH_LIST_ID, SharedPrefManager.getString(ParamConstants.SEARCH_LIST_ID,"0"));
    searchListType = extractString(getArguments(), ParamConstants.SEARCH_LIST_TYPE, SharedPrefManager.getString(ParamConstants.SEARCH_LIST_TYPE,""));
    ...
    binding.rgPickSearchType.setVisibility(View.GONE);
    binding.rgPickSearchType.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener(){
      ...
      public void onCheckedChanged(RadioGroup radioGroup, int i){
        ...
        showMarkFoundBtn = (isAllowDecode || showMarkFoundBtn || foundQty > 0 || !model.isFound) && !isActionPick && !isActionDecode && !productDao.isQtyFound(model.ean);//foundQty < model.getEanQty();
      ...
      public void onClick(View view){
        ...
        if(savedInstanceState == null && searchLog == null){
          ...
          try{
            ...
            jsonExtras.put(ParamConstants.EAN_QTY, chkZero(model.getEanQty(),model.getQty()));
          ...
          searchLog = insertSearchLog(sessionObject != null ? sessionObject.sessionId : "", chkNull(mappedEan, model.getEan()), chkZero(model.getEanQty(),model.getQty()), getSessionType().name(), searchListType, searchListId, jsonExtras);
        }
          ...
          if(isAllowDecode){
            ...
            context.showCustomAlertDialog("", String.format(getString(R.string.msg_confirm_mark_found), model.getEan()), null, getString(R.string.btn_yes), new DialogInterface.OnClickListener(){
              ...
              public void onClick(DialogInterface dialogInterface, int i){
                if(SharedPrefManager.getBoolean(ParamConstants.IS_API_BASED_MARK_FOUND_LBS,isCallAPIBasedMarkFound)) callUploadAPI();
                else{
                  ...
                  updateCount();
                  //binding.txtFound.setText(HtmlCompat.fromHtml(String.format(getString(R.string.txt_found), String.format(String.format(context.getString(R.string.txt_append_qty), context.getColorCode(model.getFoundQty() > 0 && model.getEanQty() == model.getFoundQty() ? R.color.green : model.getFoundQty() > 0 ? R.color.orange : R.color.err_red), model.getFoundQtyStr(), "" + model.getEanQty()))), HtmlCompat.FROM_HTML_MODE_LEGACY));
                  if(model.getFoundQty() >= chkZero(model.getEanQty(),model.getQty())){
  ...
  private void callUploadAPI(){
    ...
    if(isInternetConnected(context, false, isUpload)){
      try{
        ...
        requestParams.put(ParamConstants.STATUS, action);
        if(sessionObject != null)
          requestParams.put(ParamConstants.SESSION_ID, sessionObject.sessionId);
        allowBtnClick = true;
        callWebService(URLConstants.UPDATE_EXCEL_SEARCH_ITEM_FOUND, requestParams, isUpload, getString(R.string.progress_msg_uploading_data), false, false);
      }
  ...
  private void updateCount(){
    ...
    binding.txtFound.setText(HtmlCompat.fromHtml(String.format(getString(R.string.txt_found), String.format(context.getString(R.string.txt_append_qty), context.getColorCode(foundQty > 0 && chkZero(model.getEanQty(),model.getQty()) == foundQty ? R.color.green : foundQty > 0 ? R.color.orange : R.color.err_red), foundQty + "", "" + chkZero(model.getEanQty(),model.getQty()))), HtmlCompat.FROM_HTML_MODE_LEGACY));
    binding.txtPicked.setText(HtmlCompat.fromHtml(String.format(getString(R.string.txt_found), String.format(context.getString(R.string.txt_append_qty), context.getColorCode(foundQty > 0 && chkZero(model.getEanQty(),model.getQty()) == foundQty ? R.color.green : foundQty > 0 ? R.color.orange : R.color.err_red), foundQty + "", "" + chkZero(model.getEanQty(),model.getQty()))), HtmlCompat.FROM_HTML_MODE_LEGACY));
    binding.txtDecoded.setText(HtmlCompat.fromHtml(String.format(getString(R.string.txt_decoded), String.format(context.getString(R.string.txt_append_qty), context.getColorCode(decodeQty > 0 && chkZero(model.getEanQty(),model.getQty()) == decodeQty ? R.color.green : decodeQty > 0 ? R.color.orange : R.color.err_red), decodeQty + "", "" + chkZero(model.getEanQty(),model.getQty()))), HtmlCompat.FROM_HTML_MODE_LEGACY));
  ...
  public void showStyleChartAlert(){
    ...
    lvProdColors.setAdapter(new ProductColorsListAdapter((MainActivity) context, SearchListExcelStartFragment.this, styleChartAlert, lvProdSizes, listColors, model));
  ...
  protected void updateSearchUI(final int result){
    ...
    if(!showMarkFoundBtn && result >= SharedPrefManager.getInt(ParamConstants.MARK_FOUND_PERCENT_SER_LIST_BASED_NEW,AppCommonMethods.markFoundPercentNewLBS) && model.getFoundQty() <= chkZero(model.getEanQty(),model.getQty()))
    ...
    try{
      switch(url){
        ...
        case URLConstants.UPLOAD_ENCODE:
          ...
          break;
        case URLConstants.UPDATE_EXCEL_SEARCH_ITEM_FOUND:
          if(isSuccess){
            model.setFoundQty(model.getFoundQty() + 1);
            model.setFound(model.getFoundQty() > 0);
            productDao.update(model);
