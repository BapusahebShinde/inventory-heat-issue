package com.itek.retail.ui.customviews.custprogress
...
open class CircularProgressBar(context: Context, attrs: AttributeSet? = null) :
  View(context, attrs) {
  ...
  var progress: Float = 0f
    set(value) {
      field = abs(value)//if (abs(value) <= progressMax) abs(value) else progressMax
  ...
  private fun init(context: Context, attrs: AttributeSet?) {
    ...
    // Color
    progressBarColor =
      attributes.getInt(R.styleable.CircularProgressBar_cpb_progressbar_color, progressBarColor)
    ...
    // Percent View
    percentShapeEnabled =
      attributes.getBoolean(R.styleable.CircularProgressBar_cpb_percent_shape_enabled, false)
    percentShapeId = attributes.getResourceId(R.styleable.CircularProgressBar_cpb_percent_shape, 0)
    percentShapeViewSize = attributes.getDimension(
      R.styleable.CircularProgressBar_cpb_percent_shape_size,
      percentShapeViewSize
    ).pxToDp()
    percentTextViewId =
      attributes.getResourceId(R.styleable.CircularProgressBar_cpb_percent_text_view_id, 0)
    ...
    if (percentShapeEnabled) {
      percentShapeView?.let {
        if (!indeterminateMode) {
          ...
          val x =
            (radius * cos(Math.toRadians(angle + startAngle * 1.0))).toFloat() - halfPercentShape + rectF.centerX()
          val y =
            (radius * sin(Math.toRadians(angle + startAngle * 1.0))).toFloat() - halfPercentShape + rectF.centerY()
          canvas.save()
