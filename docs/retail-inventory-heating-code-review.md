...
ANR diagnostics now write separate app-wide files named `RetailAnrDiag_*.csv` and `RetailAnrDiag_*_summary.txt`. They are started from the application lifecycle, not from inventory start/stop, so main-thread responsiveness can be observed across login, navigation, inventory, search, encoding, decoding, RFID lifecycle, and other app features.

RFID lifecycle diagnostics are still recorded around existing init, release, inventory start, and inventory stop method calls without changing EPC rules, inventory acceptance, database batching, MQTT/API behavior, or RFID business logic. Existing risky areas observed during inspection include reader init/release/free calls in reader lifecycle methods and main-thread handler usage around reader power/configuration; these are now observable in the separate app-wide ANR files before attempting any behavioral refactor.

## App-wide ANR File Separation Follow-up

ANR diagnostics were separated from inventory diagnostics after review. The inventory CSV remains focused on Version 189 inventory/RF metrics, while app-wide ANR diagnostics now use independent `RetailAnrDiag_*.csv` and `RetailAnrDiag_*_summary.txt` files. The app-wide logger is initialized from `RetailApplication`, records current activity/fragment context where available, and continues independently of inventory sessions.
